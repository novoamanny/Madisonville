...
if (!function_exists('_madxmas_setup')) :
    ...
    function _madxmas_setup() {
        ...
        register_nav_menus(array(
            ...
            'footer_bottom' => esc_html__('Footer Bottom', '_s'),
...
add_action('after_setup_theme', '_madxmas_setup');
 ...
 */
function _madxmas_widgets_init() {
...
add_action('widgets_init', '_madxmas_widgets_init');

// Enqueue scripts and styles
function _madxmas_assets() {
    ...
    wp_enqueue_style('_madxmas-stylesheet-theme', get_stylesheet_uri());
    wp_enqueue_style( '_madxmas-stylesheet', get_template_directory_uri() . '/dist/css/bundle.css', array(), $ver_num, 'all' );
    if (is_front_page()) {
        wp_enqueue_style( '_madxmas-stylesheet-home-animation', get_template_directory_uri() . '/dist/css/home-animation.css', array(), '1.0.0', 'all' );
    }
    wp_deregister_script('jquery');
	wp_enqueue_script('jquery', 'https://ajax.googleapis.com/ajax/libs/jquery/3.6.1/jquery.min.js', null, null, false);
    wp_enqueue_script( '_madxmas-scripts', get_template_directory_uri() . '/dist/js/bundle.js', array(), '1.0.0', true );
    wp_enqueue_script( '_madxmas-scripts', get_template_directory_uri() . '/dist/js/custom.js', array(), '1.0.0', true );
    wp_localize_script( '_madxmas-scripts', 'acf_vars', array(
        'list_parent' => get_field( 'quote_date_management', 'option' ),
        )
    );
  }
  add_action('wp_enqueue_scripts', '_madxmas_assets');
...
function create_post_type_communities()
{
    ...
    register_post_type('communities', // Register Custom Post Type
        array(
			'labels' => array(
				'name' => __('Communties', 'communities'), // Dashboard Menu Name
				'singular_name' => __('Community', 'communities'),
				...
				'not_found' => __('No Community found', 'communities'),
				'not_found_in_trash' => __('No Community found in Trash', 'communities'),
			),
			'menu_position' =>2,
        	'public' => true,
...
add_action('init', 'create_post_type_communities');
